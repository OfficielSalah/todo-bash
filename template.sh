...
sort() {
    ...
    entry1_pos="$1"
    entry2_pos="$2"
    if [[ "$entry1_pos" -lt "1" ]] || [[ "$entry1_pos" -gt "$number_of_lines" ]]; then
        echo "Arguments are out of range"
        exit 1
    elif [[ "$entry2_pos" -lt "1" ]] || [[ "$entry2_pos" -gt "$number_of_lines" ]]; then
        echo "Arguments are out of range"
        exit 1
    fi
...
mv-entries() {
    ...
    unset nline
    for i in $(seq 1 $number_of_lines); do
        if [[ "$i" != "$entry1_pos" && "$i" != "$entry2_pos" ]]; then
            n-line "$todofile" "$i"
            echo "$nline" >>$tmp_file
        elif [ "$i" == "$entry2_pos" ]; then
...
append-deleted-tasks-to-done() {
    number-of-lines $todofile
    for task in "$@"; do
        if [ "$task" -le "$number_of_lines" ] && [ "$task" != "0" ]; then
...
delete-tasks() {
    touch tmp
    number-of-lines $todofile
    for task in $(seq 1 $number_of_lines); do
        argument-in-array $task
        if [ "$switch_aia" = "false" ] && [ "$task" -le "$number_of_lines" ] && [ "$task" != "0" ]; then
            n-line $todofile $task
            echo "$nline" >>tmp
        fi
    done
